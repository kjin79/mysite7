...
def signup(request):
    if request.method == "GET":
        ...
        #사용자의 입력이 유효한 값이 있는지 확인(아이디 중복,이메일형식,비밀번호형식확인)
        #is_valid() 함수 호출결과가 True면 정상입력, False면 유효하지 않은 값
        if form.is_valid():
            #is_valid함수의 결과가 True인경우, form.cleaned_data변수로 원본데이터를 접근할 수 있음
            #입력값에 데이터베이스에 접근하는 코드가 들어있을 수도 있기 때문에 cleaned_data를 사용 (사전형)
            #비밀번호 확인란이 같은값이 있는지 확인
            if form.cleaned_data['password'] == form.cleaned_data['password_check']:
                #User 객체 생성 및 데이터베이스에 저장
                #User 모델클래스는 비밀번호가 원본데이터가 아닌 암호화된 데이터를 저장함
                #새로운 User객체를 생성할 때 원본비밀번호를 암호화해 객체 생성해주는함수를 사용해야함 
                # = > User.objects.create_user 함수
                #create_user(id문자열,이메일,원본비밀번호문자열)
                #원본비밀번호는 암호화된 상태로 새로운 User객체를 생성 및 반환
