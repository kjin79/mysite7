...
class Category(models.Model):
    #카테고리명 / CharField는 글자수 제한해야함
    name = models.CharField('카테고리',max_length=10)
    #객체 출력 함수 오버라이딩
    def __str__(self):
        return self.name #객체안에있는 name변수를 쓰겠다.
#글 내용을 저장
class Post(models.Model):
    ...
    #User 모델클래스 외래키 설정
    #models.CASCADE : User객체가 삭제될 때, 연결된 Post객체가 같이 삭제되는 기능
    user = models.ForeignKey(User,on_delete=models.CASCADE)
    #제목
    title = models.CharField('제목',max_length=100)
    #글 내용 (글자수 제한이 없는 영역으로 지정해야함 CharField 사용 x)
    #TextFiedld : 글자수 제한이 없는 문자열 저장공간
    #blank,null : xxxField 생성자의 공통 매개변수
    #blank : form태그에서 입력받지 필수로 입력하지 않아도 되는 영역을 설정
    ...
    class Meta : 
        #정렬 순서 지정
        #정렬에 사용할 변수들을 리스트 형태로 지정. 변수 이름 앞에 -를 붙이면 내림자순으로 정렬
        #(최신 내용이 앞에와야 하니깐)
        ordering = ['-pub_date']
#글에 포함된 이미지
class PostImage(models.Model):
    ...
    #%Y : 서버시간 기준의 년
    #%m : 서버시간 기준의 월, %d : 서버시간 기준의 일
    #클라이언트가 이미지를 업로드하면 image/년/월/일 폴더에 저장함 
