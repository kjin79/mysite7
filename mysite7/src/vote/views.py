...
#@login_required : 비로그인상태일 때 해당 뷰함수를 요청하면 setting.py에 지정된 로그인 페이지로 리다이렉트함
@login_required
def vote(request):
...
# Question 객체 추가함수
# GET - 사용자에게 Question객체를 생성할 때 사용할 변수값을 입력할 수 있는
# input태그와 form태그(POST방식요청) 제공
...
def q_registe(request):
    ...
    if request.method == "GET": 
        ...
        #input태그에 아무런 값도 입력되있지 않은 상태의 객체가 생성됨
        form = QuestionForm()
        #객체를 바탕으로 HTML 코드로 변환
        #as_p,as_table,as_ul 함수 : Form 객체에 입력할 수 있는 공간들을
        #<input>로 변환하면서 <p>,<tr>과 <td>,<li>태그로 감싸주는 기능이 있는 함수
        rendered = form.as_p()
        print(rendered)
        #변환값을 HTML파일에 전달
        ...
        #form.save(여기에 아무것도 안쓰면 기본값 True로 생성됨)
        new_q = form.save()
        print(new_q)
...
# Question 객체 수정함수
# q_id : 사용자가 수정할 Question의 id값
@login_required
def q_update(request,q_id):
    #수정할 Question객체 추출
    #get_object_or_404 : 모델클래스의 객체들 중 id변수 값을 이용해 객체 한개를 추출하는 함수
    #단, 해당 id값으로 된 객체가 없는 경우 사용자의 요청에 잘못있는 걸로 판단해 404에러페이지를 전달함
    ...
    if request.method == "GET":
        ...
        #POST요청시 QuestionForm객체생성 - 수정대상객체+사용자입력
        #수정대상 객체와 사용자입력으로 폼객체 생성시 기존 객체 정보를 
        #사용자입력으로 수정한 상태의 폼객체가 생성됨
        form = QuestionForm(request.POST,instance=q)
        qq = form.save() #사용자입력으로 저장한 결과를 데이터베이스에 반영
        print(q,qq)
        #detail뷰로 이동
        return HttpResponseRedirect(
...
def q_delete(request, q_id):
    ...
    if request.method == "GET":
        ...
        print("삭제할 대상의 id변수 값", q.id)
        q.delete() #데이터베이스 삭제
        #삭제를 하더라도 id변수값을 제외한 변수들 값은 사용할 수 있음
        print("삭제된 대상의 id변수 값", q.id)
        print("삭제된 대상의 특정 title",q.title)
        return HttpResponseRedirect(reverse('main'))
    
    
#Choice객체 추가
#GET - 빈 ChoiceForm객체 생성 및 html파일 전달
#POST = 사용자입력기반의 ChoiceForm객체 생성 및 데이터베이스에 객체 저장 - detail 뷰로 이동
...
#공통 : 수정할 Choice객체를 추출
#GET - 추출한 Choice객체 기반의 ChoiceForm 객체 생성 및 html전달
#POST - 추출한 Choice객체+사용자입력 기반의 ChoiceForm객체 생성
#및 수정사항을 데이터베이스에 반영 - detail 뷰로 이동
@login_required
def c_update(request,c_id):
    c = get_object_or_404(Choice,id=c_id) #choice객체의 id가 c_id와 같은 값을 찾거나 없으면 404에러를 표시
    if request.method == "GET" :
        form = ChoiceForm(instance=c) #객체를 생성할 때 instance에 ChoiceForm에 넣어서 input처럼 만듬
        result=form.as_table()
        ...
        form = ChoiceForm(request.POST,instance=c) #instance에 수정할 대상을 넣음 instance=c
        form.save()
        return HttpResponseRedirect(
